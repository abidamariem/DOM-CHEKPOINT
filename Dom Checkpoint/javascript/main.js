        ...
        <div id="${product.id}" class="product">
          <div class="details">
            ...
            <div class="product-info">
              ...
              <p class="product-disponible">${product.disponible}</p>
            </div>
          </div>
          <div class="controls">
            <div class="quantity">
            ...
            <div class = "quantity"> <h4>Sous Total :</h4> <p class="sub-sum-num">${product.subsum} TND </p> </div>
            
            <button data-product-id="${product.id}" class="trash-btn">
              <i class="fa-solid fa-trash-can fa-lg"></i>
            </button>

            <button data-product-id="${product.id}" class="like-btn">
...
if (productsArray[productIndex].quantity === 0) {
    // remove product from DOM
  ...
  // if quantity is greater than 1,
    ...
    productsArray[productIndex].subsum =
    productsArray[productIndex].quantity * productsArray[productIndex].price;
    ...
    // update sub sum element
    const subsumNumElement = productToDecrease.querySelector(".sub-sum-num");
    subsumNumElement.textContent = productsArray[productIndex].subsum+" TND";
  }
};
...
const increaseQuantity = (incrementButton) => {
  ...
  // update sub sum element
  const subsumNumElement = productToIncrease.querySelector(".sub-sum-num");
  subsumNumElement.textContent = productsArray[productIndex].subsum + " TND";
};
...
const addEvents = () => {
  ...
  allTrashButtons.forEach((trashButton) => {
    ...
    const descriptionText = document.querySelectorAll(".product-disponible");
    descriptionText.forEach((textDisponible)=> {
      
      switch(textDisponible.textContent) {
        case "En stock":
          textDisponible.style.color = "green";
          break;
        case "En rupture":
          textDisponible.style.color = "red";
          break;
          case "En commande":
            textDisponible.style.color = "orange";
            break;
        default:
          textDisponible.style.color = "black";
      }
    });
...
const updateTotalQuantity = () => {
  ...
  if (totalQuantity === 1) {
    totalQuantityText.textContent = "article";
  } else {
    totalQuantityText.textContent = "articles";
  }
};
